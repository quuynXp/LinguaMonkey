...
# ĐỊNH NGHĨA CẤU TRÚC JSON MÀ CHÚNG TA MUỐN GEMINI TRẢ VỀ
JSON_FORMAT_PROMPT = """
...
# Phải đổi thành async vì chat_with_ai là async
async def generate_roadmap(
    ...
    """
    try:
        # 1. Tạo prompt cho người dùng (giống như file cũ)
        user_prompt = (
        ...
        # 2. Tạo "Meta-prompt" để yêu cầu Gemini trả về JSON
        final_prompt = JSON_FORMAT_PROMPT.format(user_request=user_prompt)

        # 3. Gọi Gemini (từ chat_ai.py)
        # Chúng ta dùng mảng rỗng [] cho history vì đây là tác vụ tạo mới, không phải chat
        ...
        try:
            # Xóa các ký tự thừa (nếu Gemini trả về markdown)
        ...
        # 5. Trích xuất dữ liệu từ JSON
        # (Thêm UUIDs nếu Gemini không cung cấp)
        ...
        # Lấy title và description (mô tả chung)
        ...
        # Trả về dữ liệu thật (description là mô tả chung của roadmap)
