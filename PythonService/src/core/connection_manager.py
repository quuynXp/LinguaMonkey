...
class ConnectionManager:
    ...
    async def send_to_user(self, message: dict, room_id: str, user_id: str):
        """Sends a message to a specific user in a room."""
        if room_id in self.active_connections:
            data = json.dumps(message)
            tasks = []
            for meta in self.active_connections[room_id]:
                if str(meta["user_id"]) == str(user_id):
                    tasks.append(self._safe_send(meta["ws"], data))
            if tasks: await asyncio.gather(*tasks)

    def get_participant_count(self, room_id: str) -> int:
        return len(self.active_connections.get(room_id, []))
    
    def get_remaining_users(self, room_id: str) -> List[str]:
        """Returns a list of user_ids currently in the room."""
        if room_id not in self.active_connections:
            return []
        return [str(m["user_id"]) for m in self.active_connections[room_id]]
