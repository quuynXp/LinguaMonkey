...
genai.configure(api_key=GOOGLE_API_KEY)
# Use Flash for speed. Adjust temperature to 0 for deterministic translations.
...
class HybridTranslator:
    ...
    async def translate(self, text: str, source_lang_hint: str, target_lang: str) -> Tuple[str, str]:
        ...
        is_dictionary_dump = False
        if len(text.split()) > 1:
            if len(lpm_result) > len(text) * 3: is_dictionary_dump = True
            elif "To " in lpm_result and "," in lpm_result: is_dictionary_dump = True
        
        # If coverage is high enough, return immediately
        if coverage >= 0.85 and not is_dictionary_dump:
            return lpm_result, detected_lang

        # 4. Fallback: Gemini Flash (Replaces deep_translator)
        try:
            final_text = ""
            
            # Using Gemini Flash is fast enough for short text too.
            # Structured prompt for speed.
            prompt = (
                f"Translate the following text from {detected_lang} to {target_lang}. "
                f"Output ONLY the translated text. Do not add quotes or explanations.\n"
                f"Text: {text}"
            )
            ...
            # 5. WRITE BACK (Cache warming)
            # Only cache short phrases to avoid polluting Redis with unique sentences
            if len(text.split()) < 8: 
                key = self._get_redis_key(detected_lang, text)
                await self.redis.hset(key, mapping={target_lang: final_text})
                await self.redis.expire(key, 86400 * 7) # 1 week TTL
            ...
            # Fallback to LPM result if API fails, better than nothing
            return lpm_result, detected_lang
