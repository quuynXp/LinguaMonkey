...
# SỬA: Thêm Request Transformer cho Python WS

# Mục đích: Chuyển Query Parameter 'token' thành Header 'Authorization'

# Python BE có thể đọc Header này nếu cần, hoặc đảm bảo Query Param được chuyển tiếp.

# echo ">>> Applying Request Transformer to python-ws route (Query Param to Header)..."

# curl -s -X POST $KONG_ADMIN/routes/python-ws/plugins \

#   -d "name=request-transformer" \

#   -d "config.add.headers=Authorization:Bearer \$(query_params.token)" \

#   -d "config.remove.querystring=token" || true # Xóa Query Param nếu muốn Kong xử lý JWT
