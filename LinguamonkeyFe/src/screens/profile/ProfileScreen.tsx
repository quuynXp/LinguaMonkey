...
const BadgeProgressSection = ({ userId }: { userId: string }) => {
  ...
  if (!badgeProgress || badgeProgress.length === 0) return null;
    ...
    <View style={styles.card}>
      ...
      <View style={styles.badgeGrid}>
        {badgeProgress.map((item, index) => {
          ...
          const progressPercent = Math.min(100, Math.max(0, (item.currentUserProgress / 100)));
          const imageSource = item.imageUrl ? { uri: item.imageUrl } : getCourseImage(null); // Fallback to course placeholder

          return (
            <View key={index} style={styles.badgeCard}>
              <View style={[styles.badgeImageContainer, isLocked && styles.grayscale]}>
                <Image source={imageSource} style={styles.badgeImageLarge} />
                {isLocked && (
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1, justifyContent: 'flex-end' }}>
        <Text style={[styles.infoValue, label.includes('VIP') ? { color: '#D97706', fontWeight: 'bold' } : {}]}>
          {value}
        </Text>
...
const ProfileScreen: React.FC = () => {
  ...
  const pickImage = useCallback(async () => {
    try {
      ...
      setUploading(true);
      const tempPath = await uploadTemp(file);
      if (!tempPath) throw new Error('Upload failed - No file ID returned');
      ...
      <View style={styles.singleHeader}>
        <View style={styles.characterContainer}>
          ...
          <View style={styles.flagBadge}>
            {getCountryFlag(user?.country, 20)}
          </View>
  ...
  const renderWalletCard = () => (
    <View style={styles.walletCard}>
      ...
      <View style={styles.walletActions}>
        ...
        >
          <Icon name="add" size={20} color="#064E3B" />
          ...
          <View style={styles.card}>
            ...
            <View style={styles.infoGrid}>
              <InfoRow
                icon="verified-user"
                label="VIP Status"
                value={userStore.vip ? `${userStore.vipDaysRemaining || 0} days remaining` : t('common.no')}
              />
                  ...
                  <View style={styles.languageTags}>
                    {userStore.languages.map((lang, idx) => (
                      <View key={idx} style={styles.langTag}>
                        {/* Use getCountryFlag for visual representation */}
                        <View style={{ marginRight: 4 }}>
                          {getCountryFlag(lang, 14)}
                        </View>
                        <Text style={styles.langTagText}>{lang.toUpperCase()}</Text>
                      </View>
                    ))}
                  </View>
                </View>
