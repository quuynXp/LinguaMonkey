...
// import { MemorizationResponse, MemorizationRequest, UserReminderRequest, FlashcardResponse, CreateFlashcardRequest } from "../../types/dto";
// import * as Enums from "../../types/enums";
// import { createScaledSheet } from "../../utils/scaledStyles";
// import { useUserStore } from "../../stores/UserStore";
// import ScreenLayout from "../../components/layout/ScreenLayout";
// import { TimeHelper } from "../../utils/timeHelper";
// import { FlashcardList, CommunityFlashcardList } from "../../components/flashcard/FlashcardLists";
// import CreateEditFlashcardModal from "../../components/flashcard/CreateEditFlashcardModal"; // New Component

// const { width } = Dimensions.get('window');

// // --- Helper Components: Note View and Flashcard View ---

// // Enhanced Note Item Renderer
// const renderNoteItem = ({ item, t, handleToggleFavorite, handleDelete }: { item: MemorizationResponse, t: (key: string, options?: any) => string, handleToggleFavorite: (item: MemorizationResponse) => void, handleDelete: (id: string) => void }) => (
//   <View style={styles.noteCard}>
//     <View style={styles.noteHeader}>
//       <View style={styles.noteTypeTag}>
//         <Text style={styles.noteTypeTagText}>
//           {item.contentType === Enums.ContentType.VOCABULARY ? "🔤 Word" :
...
//   // NEW: Separate content views
//   const [currentView, setCurrentView] = useState<'notes' | 'flashcards' | 'community'>('notes');
...
//   const fadeAnim = useRef(new Animated.Value(0)).current;
//   const lessonId = courseId; // For flashcards, assuming courseId is lessonId or equivalent
...
//   // Flashcard hooks
//   const { useGetMyFlashcards } = useFlashcards();
//   const { data: myFlashcardsPage, isLoading: myFlashcardsLoading, refetch: refetchMyFlashcards } = useGetMyFlashcards(lessonId, { page: 0, size: 20, query: searchQuery.length > 2 ? searchQuery : undefined });
...
//   const searchParams = useMemo(() => {
//     const params: any = { page: 0, size: 20 };
//     // Note: Only fetching ALL for basic list, filtering in memory for now
//     if (searchQuery.length > 2) params.keyword = searchQuery;
//     return params;
//   }, [searchQuery]);
...
//     switch (currentView) {
//       case 'flashcards':
//         return <FlashcardList lessonId={lessonId} searchQuery={searchQuery} isLoading={myFlashcardsLoading} data={myFlashcardsPage?.data || []} />;
//       case 'community':
//         return <CommunityFlashcardList lessonId={lessonId} searchQuery={searchQuery} />;
//       case 'notes':
//       default:
//         return memorizationsLoading ? (
//           <ActivityIndicator style={{ marginTop: 40 }} color="#37352F" />
//         ) : (
//           <FlatList
//             data={notesList}
//             renderItem={({ item }) => renderNoteItem({ item, t, handleToggleFavorite, handleDelete: handleDeleteNote })}
//             keyExtractor={item => item.memorizationId}
//             contentContainerStyle={styles.listContent}
//             ListEmptyComponent={
//               <View style={styles.emptyState}>
...
//           <Icon name="arrow-back" size={24} color="#37352F" />
//         </TouchableOpacity>
//         <Text style={styles.headerTitle}>{t("notes.title") ?? "Study Center"}</Text>
//         <TouchableOpacity onPress={() => setShowFavoritesOnly(!showFavoritesOnly)}>
//           <Icon name={showFavoritesOnly ? "star" : "star-outline"} size={24} color={showFavoritesOnly ? "#F59E0B" : "#37352F"} />
//         </TouchableOpacity>
//       </View>
...
//         <TouchableOpacity onPress={() => setCurrentView('notes')} style={[styles.tabBtn, currentView === 'notes' && styles.activeTab]}>
//           <Text style={[styles.tabText, currentView === 'notes' && styles.activeTabText]}>My Notes</Text>
//         </TouchableOpacity>
//         <TouchableOpacity onPress={() => setCurrentView('flashcards')} style={[styles.tabBtn, currentView === 'flashcards' && styles.activeTab]}>
//           <Text style={[styles.tabText, currentView === 'flashcards' && styles.activeTabText]}>My Flashcards</Text>
//         </TouchableOpacity>
//         <TouchableOpacity onPress={() => setCurrentView('community')} style={[styles.tabBtn, currentView === 'community' && styles.activeTab]}>
//           <Text style={[styles.tabText, currentView === 'community' && styles.activeTabText]}>Community</Text>
//         </TouchableOpacity>
//       </View>
...
//           <TextInput
//             style={styles.searchInput}
//             placeholder={t("notes.searchPlaceholder") ?? "Search notes or flashcards..."}
//             value={searchQuery}
//             onChangeText={setSearchQuery}
//             placeholderTextColor="#9CA3AF"
//           />
...
//       {/* Note Modal (Existing Logic) */}
//       <Modal visible={showAddNoteModal} animationType="slide" transparent>
//         <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalOverlay}>
//           <View style={styles.modalContent}>
//             <View style={styles.modalHeader}>
//               <Text style={styles.modalTitle}>{t("notes.newNote") ?? "New Note"}</Text>
...
//   backButton: { padding: 4 },
//   headerTitle: { fontSize: 18, fontWeight: '600', color: '#37352F' },
//   tabSelector: { flexDirection: 'row', justifyContent: 'space-around', paddingHorizontal: 16, paddingVertical: 10, backgroundColor: '#FFFFFF', borderBottomWidth: 1, borderBottomColor: '#E5E7EB' },
//   tabBtn: { paddingVertical: 8, paddingHorizontal: 10 },
//   activeTab: { borderBottomWidth: 2, borderBottomColor: '#37352F' },
//   tabText: { fontSize: 14, fontWeight: '500', color: '#9CA3AF' },
//   activeTabText: { color: '#37352F', fontWeight: '700' },
//   searchSection: { padding: 16, backgroundColor: '#FFFFFF' },
//   searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F7F7F5', borderRadius: 8, paddingHorizontal: 10, height: 40 },
//   searchInput: { flex: 1, marginLeft: 8, fontSize: 14, color: '#37352F' },
//   contentContainer: { flex: 1, backgroundColor: '#FBFBFA' },
...
//   noteCard: { backgroundColor: '#FFFFFF', borderRadius: 8, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#E9E9E9', shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.02, shadowRadius: 2, elevation: 1 },
//   noteHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 },
//   noteTypeTag: { backgroundColor: '#E6F3FF', borderRadius: 4, paddingVertical: 2, paddingHorizontal: 6 },
//   noteTypeTagText: { fontSize: 12, color: '#0077D6', fontWeight: '600' },
//   noteDate: { fontSize: 12, color: '#9CA3AF' },
//   noteText: { fontSize: 15, color: '#37352F', lineHeight: 22, marginBottom: 12 },
//   noteFooter: { flexDirection: 'row', alignItems: 'center', borderTopWidth: 1, borderTopColor: '#F7F7F7', paddingTop: 8, gap: 16 },
//   iconBtn: { padding: 4 },
...
//   typeBtnText: { fontSize: 13, color: '#37352F' },
//   activeTypeBtnText: { color: '#0077D6', fontWeight: '600' },
//   modalInput: { fontSize: 16, lineHeight: 24, color: '#37352F', minHeight: 80, textAlignVertical: 'top', marginBottom: 10, borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 8, padding: 10 }, // Enhanced
//   reminderSection: { backgroundColor: '#F7F7F5', padding: 12, borderRadius: 8, marginBottom: 20 },
//   reminderHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
//   reminderLabel: { fontSize: 15, fontWeight: '500', color: '#37352F' },
//   reminderControls: { marginTop: 12, borderTopWidth: 1, borderTopColor: '#E1E1E1', paddingTop: 12 },
