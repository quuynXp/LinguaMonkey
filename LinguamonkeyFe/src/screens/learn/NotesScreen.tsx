...
const NotesScreen = ({ navigation }: any) => {
  ...
  const { createMemorization, isCreating } = useCreateMemorization()
  const { updateMemorization, isUpdating } = useUpdateMemorization()
  const { deleteMemorization, isDeleting } = useDeleteMemorization()
  ...
  const handleDeleteNote = (memorization: UserMemorization) => {
    Alert.alert(t("notes.deleteConfirm"), "", [
      { text: t("common.cancel"), style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await deleteMemorization(memorization.memorization_id)
            mutateMemorizations()
          } catch (error: any) {
            Alert.alert(t("common.error"), error.message || t("errors.unknown"))
          }
  ...
  const handleToggleFavorite = async (memorization: UserMemorization) => {
    try {
      await toggleFavorite(memorization.memorization_id)
      mutateMemorizations()
    } catch (error: any) {
      Alert.alert(t("common.error"), error.message || t("errors.unknown"))
    }
  ...
  const renderNote = ({ item: memorization }: { item: UserMemorization }) => (
    <View style={styles.noteCard}>
      <View style={styles.noteHeader}>
        <View style={[styles.noteTypeIndicator, { backgroundColor: getTypeColor(memorization.content_type) }]}>
          <Text style={styles.noteTypeText}>{memorization.content_type.charAt(0).toUpperCase()}</Text>
        </View>
        <View style={styles.noteActions}>
          ...
          >
            <Icon
              name={memorization.is_favorite ? "favorite" : "favorite-border"}
              size={20}
              color={memorization.is_favorite ? "#F59E0B" : "#6B7280"}
            />
          </TouchableOpacity>
      ...
      <Text style={styles.noteContent}>{memorization.note_text}</Text>

      <View style={styles.noteFooter}>
        <Text style={styles.noteDate}>{new Date(memorization.created_at).toLocaleDateString()}</Text>
        <Text style={styles.noteType}>{t(`notes.contentTypes.${memorization.content_type}`)}</Text>
      </View>
    </View>
  )
    ...
    return (
      <SafeAreaView style={styles.container}>
        <View style={styles.errorContainer}>
          ...
          <TouchableOpacity style={styles.retryButton} onPress={() => mutateMemorizations()}>
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
        ...
        {memorizationsLoading ? (
          ...
          <FlatList
            ...
            keyExtractor={(item) => item.memorization_id}
