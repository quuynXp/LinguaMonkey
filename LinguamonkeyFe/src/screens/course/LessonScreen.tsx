...
// Updated to show Media whenever available for permitted types (e.g. Vocabulary)
export const UniversalQuestionView = ({ question }: { question: LessonQuestionResponse }) => {
  const { t } = useTranslation();
  const mediaUrl = getDirectMediaUrl(question.mediaUrl);
  ...
  return (
    <View style={styles.questionContainer}>
      {/* Always Render Media if it exists (For Vocabulary/Quiz context) */}
      {renderMedia()}

      {/* If there is a transcript (e.g. Reading passage), show it */}
      {question.transcript && (
        <View style={styles.readingPassageBox}>
          <ScrollView style={{ maxHeight: 200 }} nestedScrollEnabled>
            <Text style={styles.readingPassageText}>{question.transcript}</Text>
...
const LessonScreen = ({ navigation, route }: any) => {
  ...
  const { useLessonTest, useSubmitTest } = useLessons();
  const { useCheckWriting } = useSkillLessons(); // Only using writing check now
  const submitTestMutation = useSubmitTest();
  const checkWritingMutation = useCheckWriting();
    ...
    <ScreenLayout style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <LessonInputArea
          ...
          onAnswer={handleSubmitAnswer}
          onSkip={handleNext}
          // Pass props for Recorder if needed, but mainly for Essay/Vocab now
          isRecording={false}
          isStreaming={false}
          onStartRecording={() => { }}
          onStopRecording={() => { }}
