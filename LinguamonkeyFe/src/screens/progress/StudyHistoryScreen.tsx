...
// Import các types TỪ FILE TRUNG TÂM (Giả định bạn đã tạo file /types/api.ts)
import type { StudyHistoryResponse, StudySession, StudyStats } from "../../types/api"
...
  // Truyền cả userId và timeFilter vào hook
  const { data: studyHistory, isLoading, error } = useStudyHistory(userId, timeFilter)

  // Lỗi "Two different types" và "Property 'stats' does not exist" đã được sửa
  const studySessions: StudySession[] = studyHistory?.sessions || []
  const testResults: any[] = studyHistory?.tests || [] // Tạm thời
  const stats: StudyStats | null = studyHistory?.stats || null // Giờ 'stats' đã tồn tại
...
            {/* Lỗi 'date' (string vs Date) đã được sửa vì type 'date' giờ là string */}
...
          <View
            style={[
              styles.progressFill, // Lỗi "progressFill does not exist" sẽ được sửa trong 'styles'
              {
