...
// --- SUB-COMPONENT: QUICK PROFILE POPUP (unchanged) ---
const QuickProfilePopup = ({ visible, profile, onClose, onNavigateProfile, currentUserId, statusInfo }: any) => {
    // ... (Keep existing implementation) ...
    return null; // Placeholder for brevity, assume original code here
};

// --- SUB-COMPONENT: CHAT LANGUAGE SELECTOR (unchanged) ---
const ChatLanguageSelector = ({ selectedLanguage, onSelectLanguage, availableLanguages }: any) => {
    // ... (Keep existing implementation) ...
    return null; // Placeholder for brevity, assume original code here
};
...
const ChatInnerView: React.FC<ChatInnerViewProps> = ({
    ...
    const setCurrentViewedRoomId = useChatStore(s => s.setCurrentViewedRoomId);
    const userStatuses = useChatStore(s => s.userStatuses);
    const eagerTranslations = useChatStore(s => s.eagerTranslations); // GET EAGER CACHE
    ...
    const messages: UIMessage[] = useMemo(() => {
        return serverMessages.map((msg: any) => {
            ...
            // PRIORITY: Local Toggle (User clicked) -> Eager Store (Background) -> DB (Persisted)
            const dbTrans = msg.translatedLang === translationTargetLang ? msg.translatedText : null;
            ...
            const isAutoTranslated = autoTranslate && msg.senderId !== currentUserId;
            const hasMedia = !!(msg as any).mediaUrl || (msg as any).messageType !== 'TEXT';
            const showTranslation = !hasMedia && !!finalTranslation && (isAutoTranslated || !!localTrans || !!eagerTrans || !!dbTrans);
    ...
    const renderMessageItem = ({ item }: { item: UIMessage }) => {
        ...
        if (currentView && currentView !== 'original') {
            const eagerTrans = eagerTranslations[item.id]?.[currentView];
            const localTrans = localTranslations[item.id]?.[currentView];
            displayText = localTrans || eagerTrans || (item.translatedLang === currentView ? item.translatedText : item.text);
            isTranslatedView = true;
        } else if (displayData.isTranslated) {
            displayText = displayData.text;
            isTranslatedView = true;
